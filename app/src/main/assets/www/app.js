...
function playTrackLocal(t, idx = -1) {
    ...
    applyVol();
    LOCAL.audio.play().catch(() => toast('로컬 재생 실패'));
    applyTrackUi(t);
    _clearLyrics();
    fetchLyrics(t.id);
}
...
function _renderLsLyrics() {
    ...
    LY.lines.forEach((line, i) => {
        ...
        el.addEventListener('click', () => {
            if (LOCAL.active && LOCAL.audio) LOCAL.audio.currentTime = line.start + 0.05;
            else if (S.ytPlayer && S.ytReady) S.ytPlayer.seekTo(line.start + 0.05, true);
        });
...
async function fetchLyricsForSave(track) {
    ...
    if (res.success && res.lines?.length) return res.lines;
    return null;
}
...
async function saveLocalMusic() {
    ...
    toast('저장 중…');
    try {
        const lyrics = await fetchLyricsForSave(S.track);
        if (!lyrics?.length) {
            toast('가사를 가져오지 못했어요');
            return;
        }
...
function updSaveBtn() {
    ...
    ['b-save', 'np-save'].forEach(id => document.getElementById(id)?.classList.toggle('on', !!on));
}
...
async function fetchLyrics(videoId) {
    ...
    if (local?.lyrics?.length) {
        LY.videoId = videoId;
        LY.lines = local.lyrics.map(l => ({ start: l.start, end: l.end, text: l.text }));
        _renderLyrics();
